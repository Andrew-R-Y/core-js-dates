...
function getNextFriday(date) {
  let myDate = new Date(new Date(date).getTime() + 24 * 3600 * 1000);
  while (myDate.getDay() !== 5) {
    myDate = new Date(new Date(myDate).getTime() + 24 * 3600 * 1000);
  }
  return myDate;
}
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  const delta =
    (new Date(dateEnd) - new Date(dateStart)) / 24 / 3600 / 1000 + 1;
  return delta;
}
